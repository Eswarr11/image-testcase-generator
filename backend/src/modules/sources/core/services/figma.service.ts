...
import { fetchWithTimeout } from './atlassian-auth.service';
...
export async function fetchFigmaContent(
  ...
  logger.info('figma_fetch_start', {
    fileKey: parsed.fileKey,
    hasNodeId: Boolean(parsed.nodeId),
    selectedFrameCount: options.selectedFrameIds?.length || 0,
  });
  ...
  try {
    if (parsed.nodeId) {
      ...
      if (!file.document) {
        throw new SourceServiceError(502, 'Bad Gateway', 'Figma file had no document tree', {
          event: 'figma_fetch_fail',
          fields: { fileKey: parsed.fileKey, reason: 'no_document' },
        });
      }
      ...
      const tryExport = async (scale: number) => {
        ...
        if (!imgRes.ok) {
          if (isFigmaRateLimit(imgRes)) {
            throw new SourceServiceError(429, 'Rate Limited', figmaRateLimitMessage(), {
              event: 'figma_fetch_fail',
              fields: { fileKey: parsed.fileKey, status: 429, reason: 'rate_limit' },
            });
          }
    ...
    logger.info('figma_fetch_success', {
      fileKey: parsed.fileKey,
      frameCount: availableFrames.length,
      exportedCount: exportIds.length,
    ...
    if (err instanceof SourceServiceError) {
      if (!err.event) {
        throw new SourceServiceError(err.status, err.code, err.message, {
          event: 'figma_fetch_fail',
          fields: { fileKey: parsed.fileKey, status: err.status, code: err.code },
        });
      }
      throw err;
    }
    if ((err as Error).name === 'AbortError') {
      throw new SourceServiceError(
        ...
        {
          event: 'figma_fetch_fail',
          fields: { fileKey: parsed.fileKey, reason: 'timeout' },
        }
      );
    }
    throw new SourceServiceError(
      ...
      {
        event: 'figma_fetch_fail',
        fields: { fileKey: parsed.fileKey, reason: (err as Error).message },
      }
    );
  }
}
